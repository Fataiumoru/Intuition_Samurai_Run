...
function animatePoem(lines, delay=2400, fade=900){
  ...
  let i=0;
  function next(){
    if(i>=lines.length){ connectWalletBtn.classList.remove("hidden"); return; }
    const line=lines[i++];
...
function hitboxS(){
  return {
    x: samurai.x + samurai.w * 0.28,   // X feels good
    y: samurai.y + samurai.h * 0.55,   // lowered so feet touch first
    w: samurai.w * 0.45,
    h: samurai.h * 0.40
  };
}
